...
class Interaction:
    def ask_text(self, prompt: str, *, default: Optional[str] = None) -> Optional[str]:
...
class TUIInteraction(Interaction):
    ...
    def ask_text(self, prompt: str, *, default: Optional[str] = None) -> Optional[str]:
        ...
        try:
            if HTML:
                return self._session.prompt(HTML(f"<prompt>{prompt}</prompt> "), default=default or "").strip()
            return self._session.prompt(f"{prompt} ", default=default or "").strip()
